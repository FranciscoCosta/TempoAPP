...
const criaCartao = async (local) => {
  const dadosLocal = await verificaDados(local);
  const resultado = await buscaTempoApi(dadosLocal);
  const { main, name, wind, weather } = resultado;
  const { temp, temp_max, temp_min, feels_like, humidity } = main;

  nomeCidade.innerHTML = name;
  temperatura.innerHTML = `Temperatura é de : <span>${Math.round(
    temp
  )}Cº</span>`;
  sensacao.innerHTML = `Sensação termica é de : <span>${Math.round(
    feels_like
  )}Cº</span>`;
...
const pegaCEP = async (cidade) => {
  try {
    return fetch(`https://brasilapi.com.br/api/cep/v2/${cidade}`)
    .then(
...
const verificaDados = async (cidade) => {
  if (!isNaN(cidade)) {
    const cidadeLatLon = await pegaCEP(cidade);
    const {location:{coordinates}} = cidadeLatLon
